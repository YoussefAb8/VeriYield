...
st.markdown("""
    ...
    h1, h2, h3 { font-family: 'Orbitron', sans-serif; color: #00ff9d; }
    .glass-card {
...
with st.sidebar:
    ...
    st.divider()
    st.caption("VeriYield v2.5 • 0.1 OPG Mode")
...
st.caption("LIVE DEFI INTELLIGENCE • POWERED BY TEE")
st.divider()
...
# TAB 1: OPTIMIZER - OPTIMIZED FOR ONLY 0.1 OPG
with tab_optimizer:
    st.markdown("### 💰 Generate Verifiable 2026 Yield Strategy")
    st.caption("⚠️ Running in **0.1 OPG mode**. Using shorter responses to minimize cost.")
    ...
    if st.button("🚀 GENERATE VERIFIABLE STRATEGY", type="primary"):
        if not asset.strip():
            ...
            with st.spinner(f"Analyzing {asset.upper()} in TEE... (0.1 OPG mode)"):
                try:
                    PRIVATE_KEY = st.secrets.get("OG_PRIVATE_KEY")
                    if not PRIVATE_KEY:
                        st.error("OG_PRIVATE_KEY is missing in Streamlit Secrets. Please add it.")
                        st.stop()
                   
                    llm = og.LLM(private_key=PRIVATE_KEY)
                    
                    # === As requested: min_allowance exactly 0.1 ===
                    with st.spinner("Setting minimal OPG allowance (0.1)..."):
                        approval = llm.ensure_opg_approval(min_allowance=0.1)
                    
                    if hasattr(approval, 'tx_hash') and approval.tx_hash:
                        st.success("✅ OPG allowance set to 0.1")
                    else:
                        st.caption("✅ Using existing 0.1 OPG allowance")
                    
                    # Cost-optimized instructions
                    detail_instructions = {
                        "Concise": "Keep response short and clear.",
                        "Standard": "Provide balanced, informative response with good detail.",
                        "Detailed": "Provide detailed analysis with explanations."
                    }
                    ...
                    You are VeriYield AI, a professional 2026 DeFi strategist.
                    Create a yield strategy for {asset.upper()}.
                    
                    Use this structure:
                    1. Executive Summary
                    2. Strategy Matrix (markdown table: Protocol | Mechanism | Est. APY | Rationale)
                    3. Key Risks & Mitigations
                    4. Composability Notes
                    ...
                    result = asyncio.run(
                        llm.chat(
                            ...
                            messages=[
                                {"role": "system", "content": "You are VeriYield AI — professional DeFi strategist. Be clear, useful, and focused."},
                                {"role": "user", "content": query}
                            ],
                            max_tokens=800,        # Reduced to work with 0.1 OPG
                            temperature=0.6
                        )
                    )
                    ...
                    st.success(f"🎯 {asset.upper()} Strategy • {risk_level} Risk • {detail_level}")
                    
                    word_count = len(content.split())
                    st.caption(f"📝 ~{word_count} words | Optimized for 0.1 OPG")
                    ...
                    if tx_hash:
                        with st.expander("🛡️ View TEE Hardware Attestation"):
                            st.code(tx_hash[:66] + "..." if len(str(tx_hash)) > 66 else tx_hash, language="plaintext")
                            st.caption("Verified in Trusted Execution Environment")
                   
                except Exception as e:
                    error_str = str(e).lower()
                    st.error(f"Error generating strategy: {str(e)}")
                    if any(word in error_str for word in ["payment", "allowance", "insufficient", "balance"]):
                        st.warning("💰 Not enough OPG for this call.")
                        st.info("→ Claim more free test OPG from: https://faucet.opengradient.ai")
                    else:
                        st.info("Try a lower detail level or claim more OPG.")
...
# FOOTER
st.divider()
st.markdown(f"<p style='text-align:center; color:#64748b; font-size:0.85rem;'>VeriYield v2.5 • 0.1 OPG Mode • {current_date}</p>", unsafe_allow_html=True)
